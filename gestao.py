...
def save_file_to_firebase(db, empenho_id, file_obj):
    if db is None or file_obj is None:
        return
    try:
        ...
        db.collection("pdde_arquivos").document(empenho_id).set(
            {"file_name": file_obj.name, "file_data": b64_string}
        )
        return True
    except Exception as e:
...
def save_global_programs_to_firebase(db, lista_programas):
    if db is None:
        return
    try:
        db.collection("pdde_dados_gerais").document("programas_globais").set(
...
def get_saldo_anterior(account_name, programa, tipo_recurso, mes_alvo, ano_alvo):
    ...
    if programa in saldos_iniciais:
        val = (
            saldos_iniciais[programa].get(tipo_recurso, 0.0)
            if tipo_recurso != "Total"
            else saldos_iniciais[programa].get("Capital", 0.0)
            + saldos_iniciais[programa].get("Custeio", 0.0)
        )
        saldo += val
...
def sidebar_config():
    ...
    if st.sidebar.button(
        "🔄 Recarregar Dados", help="Use se notar dados desatualizados"
    ):
                    ...
                    if st.button("✏️ Renomear", type="primary"):
                        if novo_nome_conta and novo_nome_conta not in contas_existentes:
                            success = rename_account_in_firebase(
                                st.session_state["db_conn"], conta_alvo, novo_nome_conta
                            )
                            if success:
            ...
            with tab_del:
                if contas_existentes:
                    conta_del = st.selectbox(
                        "Apagar Conta:", contas_existentes, key="sel_del_acc"
                    )
                    if st.button(f"🗑️ Excluir {conta_del}", type="primary"):
                        if conta_del in st.session_state["accounts"]:
                            ...
                            delete_account_from_firebase(
                                st.session_state["db_conn"], conta_del
                            )
...
def calcular_rateio_rendimento(
    ...
    for prog, valores in dados_entrada.items():
        ...
        fator_cap = (
            base_prog["Capital"] / total_saldo_conta if total_saldo_conta > 0 else 0
        )
        fator_cus = (
            base_prog["Custeio"] / total_saldo_conta if total_saldo_conta > 0 else 0
        )
...
def render_financeiro_view(conta_atual, ano_atual, programas):
    ...
    with tab_lanc:
        ...
        val_rendimento_inicial = (
            sum([m["total_rendimento"] for m in registros_existentes])
                    ...
                    [
                        {
                            "Programa": "TOTAL",
                            "Mês": "---",
                            "Créd. Cap.": df_prog["Créd. Cap."].sum(),
                            "Créd. Cust.": df_prog["Créd. Cust."].sum(),
                            "Créd. Total": df_prog["Créd. Total"].sum(),
                            "Rend. Cap.": df_prog["Rend. Cap."].sum(),
                            "Rend. Cust.": df_prog["Rend. Cust."].sum(),
                            "Rend. Total": df_prog["Rend. Total"].sum(),
                            "Déb. Cap.": df_prog["Déb. Cap."].sum(),
                            "Déb. Cust.": df_prog["Déb. Cust."].sum(),
                            "Déb. Total": df_prog["Déb. Total"].sum(),
    ...
    with tab_resumo:
        ...
        prog_demo = st.selectbox(
            ...
            options=programas,
            key=f"sel_demo_{conta_atual}_{ano_atual}",
        )
        ...
        if prog_demo:
            ...
            with st.expander("📝 Ajustar Recursos Próprios e Devoluções"):
                ...
                with c1:
                    ...
                    n_rpc = st.number_input(
                        "Custeio",
                        value=extras.get("rec_prop_cust", 0.0),
                        key=f"rpc_{prog_demo}_{ano_atual}",
                    )
                    n_rpcap = st.number_input(
                        "Capital",
                        value=extras.get("rec_prop_cap", 0.0),
                        key=f"rpcap_{prog_demo}_{ano_atual}",
                    )
                with c2:
                    st.caption("Devolução de Recursos (12)")
                    n_dc = st.number_input(
                        "Custeio ",
                        value=extras.get("devol_cust", 0.0),
                        key=f"dc_{prog_demo}_{ano_atual}",
                    )
                    n_dcap = st.number_input(
                        "Capital ",
                        value=extras.get("devol_cap", 0.0),
                        key=f"dcap_{prog_demo}_{ano_atual}",
                    )

                if st.button(
                    "Salvar Ajustes", key=f"btn_ajuste_{prog_demo}_{ano_atual}"
                ):
                    conta_dados["extra_fields"][prog_demo] = {
                        "rec_prop_cust": n_rpc,
                        "rec_prop_cap": n_rpcap,
                        "devol_cust": n_dc,
                        "devol_cap": n_dcap,
                    }
                    save_account_to_firebase(
            ...
            s_reprog_cap = get_saldo_anterior(
                conta_atual, prog_demo, "Capital", 1, ano_atual
            )
            ...
            movs_demo = [
                m
                for m in movs
                if m["programa"] == prog_demo and m.get("ano") == ano_atual
            ]
            ...
            total_rec_cust = (
                s_reprog_cust + cred_cust + rec_prop_cust + rend_cust - devol_cust
            )
            total_rec_cap = (
                s_reprog_cap + cred_cap + rec_prop_cap + rend_cap - devol_cap
            )
                ...
                [
                    {
                        ...
                        "Custeio": s_reprog_cust,
                        "Capital": s_reprog_cap,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "Custeio": rec_prop_cust,
                        "Capital": rec_prop_cap,
                    },
                    {
                        "Descrição": "11 - Rendimento de Aplicação",
                        "Custeio": rend_cust,
                        "Capital": rend_cap,
                    },
                    {
                        "Descrição": "12 - Devolução de Recursos (-)",
                        "Custeio": devol_cust,
                        "Capital": devol_cap,
                    },
                    {
                        "Descrição": "13 - VALOR TOTAL RECEITA",
                        "Custeio": total_rec_cust,
                        "Capital": total_rec_cap,
                    },
                    {
                        "Descrição": "14 - Despesas Realizadas",
                        "Custeio": desp_cust,
                        "Capital": desp_cap,
                    },
                    {
                        "Descrição": "15 - Saldo a Reprogramar",
                        "Custeio": saldo_final_cust,
                        "Capital": saldo_final_cap,
                    },
                ]
            )
            ...
            def highlight_demo_rows(row):
                if "13 - VALOR" in row["Descrição"] or "15 - Saldo" in row["Descrição"]:
                    return [
                        "background-color: #e0f2f1; color: black; font-weight: bold"
                    ] * len(row)
                return [""] * len(row)
            ...
            st.dataframe(
                df_demo_display.style.apply(highlight_demo_rows, axis=1),
                use_container_width=True,
                height=350,
            )
        ...
        with st.expander("Ver Resumo Geral de Todos os Programas"):
            ...
            for prog in programas:
                ...
                movs_ano = [
                    m
                    for m in movs
                    if m["programa"] == prog and m.get("ano") == ano_atual
                ]
                ...
                ajuste_entradas = extras_p.get("rec_prop_cust", 0) + extras_p.get(
                    "rec_prop_cap", 0
                )
                ajuste_saidas = extras_p.get("devol_cust", 0) + extras_p.get(
                    "devol_cap", 0
                )
            ...
            if dados_resumo:
                ...
                st.dataframe(
                    df_resumo_display.style.apply(highlight_total_resumo, axis=1),
                    use_container_width=True,
                )
...
def render_resumo_consolidado_view():
    ...
    for nome_conta, dados_conta in st.session_state["accounts"].items():
        ...
        for p in progs:
            ...
            ajuste_entradas_conta += extras_p.get("rec_prop_cust", 0) + extras_p.get(
                "rec_prop_cap", 0
            )
            ajuste_saidas_conta += extras_p.get("devol_cust", 0) + extras_p.get(
                "devol_cap", 0
            )
    ...
    col1.metric(
        "Total Receita (Disp.)", format_currency(total_recebido), delta_color="normal"
    )
    col2.metric(
        "Total Saídas (Débitos)", format_currency(total_gasto), delta_color="inverse"
    )
...
def render_empenhos_global_view():
    ...
    with st.expander("⚙️ Cadastrar/Gerenciar Programas"):
        ...
        if c_p2.button("Cadastrar", key="btn_add_prog_global"):
            if (
                novo_prog_global
                and novo_prog_global not in st.session_state["global_programs"]
            ):
    ...
    if st.session_state["empenho_mode"] == "list":
        ...
        str_anos = [str(a) for a in anos_disp]
        ano_filtro = st.radio(
            "Filtrar por Ano:", str_anos, horizontal=True, index=len(str_anos) - 1
        )

        lista_programas = st.session_state["global_programs"]
        if not lista_programas:
            lista_programas = ["Sem cadastro"]
        filtro_prog_emp = st.selectbox(
            "Filtrar por Programa", ["Todos"] + lista_programas, key="filt_gemp"
        )
        ...
        if lista_final:
            ...
            c2.markdown(
                "<div class='row-header'>Programa</div>", unsafe_allow_html=True
            )
        ...
        val_data_nf = (
            safe_date(
                dados_edicao.get(
                    "data_nota_fiscal", dados_edicao.get("data_utilizacao", "")
                )
            )
            if is_edit_mode
            else None
        ...
        if not lista_programas:
            lista_programas = ["Sem cadastro"]
        prog_index = (
            lista_programas.index(val_prog)
        ...
        with st.container(border=True):
            ...
            e_prog = ce1.selectbox(
                "Programa", options=lista_programas, index=prog_index, key="form_prog"
            )
            e_num = ce2.text_input("Nº Empenho", value=val_num, key="form_num")
            e_data = ce3.date_input(
                "Data do Empenho", value=val_data, format="DD/MM/YYYY", key="form_data"
            )

            ce4, ce5, ce6 = st.columns(3)
            e_ob = ce4.text_input("Ordem Bancária (OB)", value=val_ob, key="form_ob")
            e_data_ob = ce5.date_input(
                "Data da OB", value=val_data_ob, format="DD/MM/YYYY", key="form_data_ob"
            )
            e_valor = ce6.number_input(
                "Valor (R$)",
                value=val_valor,
                min_value=0.0,
                step=0.01,
            ...
            e_status = ce7.selectbox(
                "Status",
                status_opts,
                index=status_opts.index(val_status) if val_status in status_opts else 1,
                key="form_status",
            )
            e_data_nf = None
            ...
            if is_edit_mode and file_info:
                st.markdown(
                    f"<div class='download-box'><b>Arquivo atual:</b> {file_info.get('file_name', 'arquivo.pdf')}</div>",
                    unsafe_allow_html=True,
                )
                ...
                e_file = st.file_uploader(
                    "Substituir arquivo (Opcional)", type=["pdf"], key="form_file"
                )
            else:
                e_file = st.file_uploader(
                    "Fazer upload de PDF (Máx 2MB)", type=["pdf"], key="form_file"
                )
            st.markdown("---")
            ...
            def run_save():
                ...
                payload = {
                    "programa": e_prog,
                    "numero_empenho": e_num,
                    "data_empenho": str_d_emp,
                    "ordem_bancaria": e_ob,
                    "data_ob": str_d_ob,
                    "valor": e_valor,
                    "data_nota_fiscal": str_d_nf,
                    "status": e_status,
                    "itens": e_itens,
                    "observacao": e_obs,
                }
                ...
                with c_act3:
                    with st.popover("🗑️ Excluir"):
                        ...
                        if st.button("Sim, excluir permanentemente"):
                            ...
                            st.session_state["empenhos_global"] = [
                                e
                                for e in st.session_state["empenhos_global"]
                                if e.get("id") != t_id
                            ]
